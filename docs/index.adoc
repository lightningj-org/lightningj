...
* LND: Support Macaroons Authentication.
...
Currently is Macaroon authentication not implemented and LND must be started
with the _--no-macaroon_ flag set.
...
SynchronousLndAPI synchronousLndAPI = new SynchronousLndAPI("localhost",10001,new File("/Library/Application Support/Lnd/tls.cert"));
...
AsynchronousLndAPI asynchronousLndAPI = new AsynchronousLndAPI("localhost",10001,new File("/Users/philip/Library/Application Support/Lnd/tls.cert"));
